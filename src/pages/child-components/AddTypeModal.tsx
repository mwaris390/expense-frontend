...
import { addType, updateType } from "../../hooks/slices/typeSlice";
...
function AddTypeModal({ isOpen, onClose, userId, updateData }: ModalProps) {
  ...
  const submitModal = async (data: schemaType) => {
    ...
    try {
      if (updateData == undefined) {
        await dispatch(addType({ userId, data }) as any);
      } else {
        let typeId: string = updateData.id;
        await dispatch(updateType({ id: typeId, data }) as any);
      }
      reset();
      onClose();
      ...
      <dialog ref={dialogRef} className=" rounded-lg p-6 w-1/2 mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">
            {updateData == undefined ? "Add Type" : "Update Type"}
          </h2>
          <button
            onClick={onClose}
